...
- **`special-projects.html` — Pledge/Direct Contribution funding entries never wrote an `income` doc at all.** `commitFundingDraftItem()`'s non-Church-Fund branch (`sourceType` = Pledge or Direct Contribution) only ever created a `specialProjectEntries` tracking doc — despite the form capturing a real `receiveAccount` the money was received into, no `income` document was ever created, so that cash was permanently invisible to `financial-overview.html` (which only listens to `income`/`expenses`). Confirmed by the fact that `linkedIncomeId` was read/deleted in three cascade functions but never set anywhere in the file. Fixed by creating a batched `income` doc (`targetAccount`/`targetAccId` = `receiveAccount`) alongside the entry doc. The edit path (`saveFundingEntry()`) had the same gap in reverse — it deleted the linked income doc on every edit without ever recreating it, silently erasing previously-recorded cash each time a pledge entry was edited; fixed to update the existing income doc in place (or create one now, for entries saved before this fix), batched with the entry/expense updates instead of a bare sequential delete.
- **Retry-duplication risk in both bulk-save loops (minor, but same "cash records don't match" symptom family).** `tithe-entry.html` `submitAllEntries()` and `special-projects.html` `saveFundingDraft()` both looped over a draft array with sequential `await` writes and only cleared the draft state after the entire loop finished. If entry N of M failed partway through, entries `1..N-1` were already committed to Firestore but still sitting in the draft list, so retrying "Save" would resubmit them as duplicates — inflating recorded income. Fixed both to remove each item from the draft array as it individually succeeds, so a retry after a partial failure only resubmits what's actually left.
