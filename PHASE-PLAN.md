...
Status: Done (code side); deploy still blocked. Four autonomous-loop runs (2026-07-18, 2026-07-20, 2026-07-21, 2026-07-22) all received HTTP 401 from Firebase when attempting `npx firebase-tools@15 deploy --token ...`. The provided `FIREBASE_TOKEN` has expired. **Action required from owner**: provide authentication via one of: (a) a fresh `firebase login:ci` token (run `firebase login:ci` locally and update `FIREBASE_TOKEN` in the scheduled task), or (b) a service account key stored as `GOOGLE_APPLICATION_CREDENTIALS` (recommended — see https://cloud.google.com/docs/authentication/getting-started). Until then, `certificate-of-giving.html`, `verify-certificate.html`, and `equipment.html` are on main/origin but not yet live. Live smoke test against real member/income data still needed after deploy (operational task for Treasurer). **No further automated runs can make progress until new credentials are supplied.**
