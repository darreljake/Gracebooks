...
Status: Done (code side); deploy blocked — Firebase deploy token has expired (401 on both hosting and firestore:rules). A new token is needed before this can go live. Live smoke test against real member/income data still needed after deploy (operational task for Treasurer).
