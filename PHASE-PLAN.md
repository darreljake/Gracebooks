...
Status: Done (code side); needs Firestore rules deploy and a live smoke test against real member/income data before Treasurer use.
...
- **Not yet done / needs a human before first real use**: `firestore.rules` changes above still need `firebase deploy --only firestore:rules --project gracebooks-7eebc`; the existing equipment inventory this feature was built for still needs to be entered via the CSV import or the manual form (no seed data was available in this environment); `settings/signatories` should be reviewed so the Treasurer/Pastor name+title shown on certificates match who's actually signing.
