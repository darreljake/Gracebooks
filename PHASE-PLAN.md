...
Status: Done (code side); deploy still blocked. Two successive autonomous-loop runs (2026-07-18 and 2026-07-20) both received HTTP 401 from Firebase when attempting `npx firebase-tools@15 deploy --token ...`. The `--token` flag is deprecated in firebase-tools v15 and will be removed in a future major version. **Action required from owner**: provide authentication via one of: (a) a fresh `firebase login:ci` token if `--token` still works on the current project, or (b) a service account key stored as `GOOGLE_APPLICATION_CREDENTIALS` (recommended — see https://cloud.google.com/docs/authentication/getting-started). Until then, `certificate-of-giving.html`, `verify-certificate.html`, and `equipment.html` are on main/origin but not yet live. Live smoke test against real member/income data still needed after deploy (operational task for Treasurer).
