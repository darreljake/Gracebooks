...
- Expense category classification at approval time: Done, closing the gap where every release only ever landed under one generic placeholder category with no way to route it into the church's real budget categories for Reports/Print Report.
  ...
  - **Balance settlement** — if the liquidated total is less than what was released, an `income` doc (`Cash Advance Return`) is recorded crediting the same fund back for the unspent amount; if the staff member overspent, an additional classified `expenses` doc is recorded for the shortfall (Treasurer picks its category too). Net cash impact always matches what was actually spent once reconciled.
  - Requests store `classifiedExpenseIds`/`returnedIncomeId`/`topupExpenseId` for traceability; the details modal shows each item's assigned category and how any balance was settled once reconciled.
